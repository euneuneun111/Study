...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 저장 버튼 클릭 리스너 추가
        // 저장 버튼을 눌렀을 때 실행되는 코드
        Button saveButton = findViewById(R.id.save_button);  // 저장 버튼이 있는 레이아웃에서 ID를 가져옴
        saveButton.setOnClickListener(v -> {
            if (selectedImageUri != null && predictions != null && predictions.length > 0) {
                ...
                mainIntent.putExtra("predictionResult", predictions[0].className); // 가장 높은 유사성이 있는 병명 전달
